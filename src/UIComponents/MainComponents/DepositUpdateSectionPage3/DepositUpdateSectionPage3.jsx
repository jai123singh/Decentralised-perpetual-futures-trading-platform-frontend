...
export default function DepositUpdateSectionPage3({
  goToPageOne,
  ...
  function findError(value) {
    ...
    } else if (valueAsBigNumber.isGreaterThan(maxWithdrawableDepositInEth)) {
      error = `Amount cannot be more than ${maxWithdrawableDepositInEth.toString()}`;
    } else {
      let parts = value.split(".");
        ...
        if (page == "withdraw") {
          return (
            <>
              ...
              <Button
                className={isDisabled ? "withdraw-eth-disable-style" : ""}
                disabled={isDisabled}
                onClick={handleClick}
              >
                Withdraw ETH
